...
from app.rag.embeddings import EMBEDDING_MODEL_NAME, embed_text, vector_literal
...
from app.services.document_file_store import document_file_store
...
def process_document(document_id: str) -> None:
    ...
    try:
        ...
        document_repository.replace_pages(document_id, pages)
        ...
        try:
            ...
            model_name = None
        document_repository.upsert_generated_metadata(document_id, metadata, model_name)
...
async def save_upload(upload: UploadFile) -> dict:
    ...
    return document_repository.to_metadata(document_repository.get_record(document_id))
...
def list_documents(
    ...
    return [document_repository.to_metadata(row) for row in rows]
...
def get_document_detail(identifier: str, include_restricted: bool = False) -> dict:
    return document_repository.get_detail_record(identifier, include_restricted=include_restricted)


def get_document_chunks(identifier: str, include_restricted: bool = False) -> list[dict]:
    return document_repository.list_chunk_records(identifier, include_restricted=include_restricted)
